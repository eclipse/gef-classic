...
protected int findIndexOfTreeItemAt(com.ibm.etools.draw2d.geometry.Point pt){
	int index = -1;
...
private TreeItem findTreeItemAt(com.ibm.etools.draw2d.geometry.Point pt){
...
private boolean isInUpperHalf(Rectangle rect, 
					   com.ibm.etools.draw2d.geometry.Point pt){
...
private void showAddFeedback(Request req){
	...
	Tree tree = getTree();
	com.ibm.etools.draw2d.geometry.Point pt = request.getLocation();
	TreeItem item = findTreeItemAt(pt);
	if(item == null){
		Widget widget = ((TreeEditPart)getHost()).getWidget();
		if(widget == tree){
