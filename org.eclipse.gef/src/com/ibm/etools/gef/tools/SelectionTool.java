...
protected boolean handleKeyDown(KeyEvent e) {
	...
	if (acceptTraverseHandle(e)) {
		if (stateTransition(STATE_ACCESSIBLE_DRAG_IN_PROGRESS, STATE_ACCESSIBLE_DRAG)){
			if (getDragTracker() != null)
...
protected boolean handleMove() {
	if (isInState(STATE_INITIAL)){
		...
		showTargetFeedback();
		return true;
	} else if (isInState(STATE_ACCESSIBLE_DRAG)){
		EditPartViewer viewer = getCurrentViewer();
		if (viewer instanceof GraphicalViewer) {
...
private boolean handleTraverseHandle(KeyEvent e){
	 ...
	 */
	stateTransition(STATE_INITIAL, STATE_ACCESSIBLE_DRAG);
