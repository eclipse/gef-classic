...
public abstract class WorkbenchPartAction extends Action implements Disposable, UpdateAction {
	...
	/**
	 * Returns <code>true</code> if the action is enabled. If the action determines
	 ...
	 * <P>
	 * Sometimes a value of <code>false</code> can be used to improve performance
	 * and reduce the number of times {@link #calculateEnabled()} is called.
	 * However, the client must then call the {@link #update()} method at the proper
	 * times to force the update of enablement.
	 * <P>
	 * Sometimes a value of <code>true</code> can be used to improve performance. If
	 * an <code>Action</code> only appears in a dynamic context menu, then there is
	 * no reason to agressively update its enablement status because the user cannot
	 * see the Action. Instead, its enablement only needs to be determined when
