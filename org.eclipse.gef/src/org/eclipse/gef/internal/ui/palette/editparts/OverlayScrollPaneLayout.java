...
public class OverlayScrollPaneLayout extends ScrollPaneLayout {
	...
	/**
	 * {@inheritDoc} In OverlayScrollPane, scrollbars are overlayed on top of the
	 * Viewport, so the preferred size is just the Viewports preferred size.
	 * 
	 * @since 2.0
	 */
	protected Dimension calculatePreferredSize(IFigure container, int wHint, int hHint) {
		...
		return scrollpane.getViewport().getPreferredSize(wHint - excludedWidth, hHint - excludedHeight)
	...
	public void layout(IFigure parent) {
		...
		Dimension available = clientArea.getSize(),
				preferred = viewport.getPreferredSize(available.width, available.height).getCopy();
		...
		if (showV) {
			bounds = new Rectangle(viewportArea.right() - insets.right, viewportArea.y, insets.right,
					viewportArea.height);
			vBar.setBounds(bounds);
			// vBar.setMaximum(preferred.height);
		}
		if (showH) {
			bounds = new Rectangle(viewportArea.x, viewportArea.bottom() - insets.bottom, viewportArea.width,
					insets.bottom);
