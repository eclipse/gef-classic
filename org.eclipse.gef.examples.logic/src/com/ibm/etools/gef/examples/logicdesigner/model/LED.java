	...
	if(descriptors!=null){
		...
		newDescriptors[descriptors.length]=
			new TextPropertyDescriptor(P_VALUE, 
				LogicResources.getString("PropertyDescriptor.LED.Value")); //$NON-NLS-1$
	}else{
...
public String toString() {
	return LogicResources.getString("LED.LabelText") + " #" + getID() //$NON-NLS-2$//$NON-NLS-1$
	       + " " + LogicResources.getString("PropertyDescriptor.LED.Value") //$NON-NLS-2$//$NON-NLS-1$
	       + "=" + getValue();    //$NON-NLS-1$
}
