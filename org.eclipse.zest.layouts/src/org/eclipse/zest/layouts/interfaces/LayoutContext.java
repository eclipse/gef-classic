/*******************************************************************************
 * Copyright (c) 2009, 2023 Mateusz Matela and others.
 *
...
public interface LayoutContext {
	...
	/**
	 * Returns all the connections between nodes that should be laid out. Replacing
	 ...
	 * subgraphs and unpruned nodes. Replacing elements in the returned array does
	 ...
	 * given entity is a subgraph, connections adjacent to each of its nodes will be
	 * included in the result. All the undirected nodes connecting the two nodes
	 * will be also included in the result. Replacing elements in the returned array
	...
	/**
	 *
	 * @return true if a layout algorithm is allowed to place graph elements outside
	 *         of suggested bounds
	 */
	 ...
	 * given nodes already belong to another subgraphs, they are removed from them
	 * prior to adding to the new subgraph.
	 *
	 * @param nodes nodes to add to the new subgraph
	 */
	...
	/**
	 * Checks if this layout context allows layout algorithms to work continuously
	 * in background and change the layout with time or in reaction to some events.
	 * If background changes are not allowed, a layout algorithm can make changes in
	 * layout context only when {@link LayoutAlgorithm#applyLayout(boolean)} is
	 * called (otherwise a runtime exception will be thrown).
	 *
	...
	/**
	 * Sets the expand/collapse manager for this context. The manger will be used to
	 * handle expansion related methods called on the owner of this context.
	 *
	...
	/**
	 * Adds a listener to the context that will be notified about changes in this
	 * context's layout, that is movement and resizing of nodes / subgraphs. The
	 * notifications will not include changes made with API included in layout
	 * related interfaces, so that layout algorithms won't be notified about changes
	...
	/**
	 * Adds a listener to the context that will be notified about changes in graph
	 * structure, that is addition and removal of nodes and connections. The
	 * notifications will not include changes made with API included in layout
	 * related interfaces, so that layout algorithms won't be notified about changes
	 * they invoke. Only internal changes of the system will fire events.
	 *
	...
	/**
	 * Adds a listener to the context that will be notified about changes related to
	 * its configuration.
	 *
	 * @param listener listener to add
	 */
	...
	/**
	 * Adds a listener to the context that will be notified about changes in graph
	 * pruning, that is hiding and showing of nodes. The notifications will not
	 * include changes made with API included in layout related interfaces, so that
	 * layout algorithms won't be notified about changes they invoke. Only internal
	 * changes of the system will fire events.
	 *
	 * @param listener listener to add
	 */
	 ...
	 * display.
	 *
	 * @param animationHint a hint for display mechanism indicating whether changes
	 *                      are major and should be animated (if true) or not.
	 */
