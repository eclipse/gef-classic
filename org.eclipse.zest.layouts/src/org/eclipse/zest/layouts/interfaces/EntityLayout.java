...
public interface EntityLayout {
	...
	/**
	 * Returns a point laying in the center of this entity. Any subsequent changes
	...
	/**
	 * Sets the position of this entity. The node will be moved so that it's center
	 * is located in the given point.
	 *
	...
	/**
	 * Returns aspect ratio that is preferred for this entity. Can be 0 if this node
	 * can't be resized anyway or it doesn't care about about its ratio.
	 *
	 ...
	 * <li>all subgraphs that are <code>GraphEntities</code> and contain at least
	 * one successor node of node N</li>
	 * </ul>
	 * Successor entities of a subgraph S that is a <code>GraphEntity</code> are:
	 * <ul>
	 * <li>all unpruned nodes that are successor of at least one node from subgraph
	 * S</li>
	 * <li>all subgraphs that are <code>GraphEntities</code> and contain at least
	...
	/**
	 * Returns all entities that are direct predecessors of this entity. Predecessor
	 * entities of an unpruned node A are:
	 * <ul>
	 * <li>all unpruned predecessor nodes of node N</li>
	 * <li>all subgraphs that are <code>GraphEntities</code> and contain at least
	 * one predecessor node of node N</li>
	 * </ul>
	 * Successor entities of a subgraph S that is a <code>GraphEntity</code> are:
	 * <ul>
	...
	/**
	 * Returns all graph items that are represented using this single entity. They
	 * are useful when a layout would get information about the graph it draws.
	 *
