...
public class TreeLayoutObserver {
	...
	public static class TreeNode {
		...
		/**
		 *
		 * @return height of this node in the tree (the longest distance to a leaf, 0
		 *         for a leaf itself)
		 */
		...
		/**
		 *
		 * @return depth of this node in the tree (distance from root, 0 for a root and
		 *         -1 for {@link TreeLayoutObserver#getSuperRoot() Super Root}
		 */
		public int getDepth() {
			return depth;
		}
		...
		/**
		 *
		 * @return number of all leaves descending from this node (1 for a leaf itself)
		 */
		...
		/**
		 * Returns order in which nodes are visited during Deep First Search. Children
		 * are visited in the same order as they were added to their layout context,
		 * unless {@link TreeLayoutObserver#recomputeTree()} was called after the nodes
		 * were added. In that case the order is determined by order of nodes returned
		 * by {@link NodeLayout#getSuccessingNodes()}. Leaves are assigned successive
		 * numbers starting from 0, other nodes have order equal to the smallest order
		 * of their children.
		 *
		...
		/**
		 *
		 * @return true if this node is the first child of its parent (has the smallest
		 *         order)
		 */
		 ...
		 * @param owner <code>TreeLayoutObserver</code> owning created node
		 */
		...
		/**
		 * Adds given node to the list of this node's children and set its parent to
		 * this node.
		 *
		...
		/**
		 * Performs a DFS on the tree structure and calculates all parameters of its
		 * nodes. Should be called on {@link TreeLayoutObserver#getSuperRoot() Super
		 * Root}. Uses recurrence to go through all the nodes.
		 */
		protected void precomputeTree() {
			if (children.isEmpty()) {
				...
				numOfLeaves = 0;
				numOfDescendants = 0;
				for (ListIterator<TreeNode> iterator = children.listIterator(); iterator.hasNext();) {
		...
		/**
		 * Finds a node that is the best parent for this node. Add this node as a child
		 * of the found node.
		 */
		protected void findNewParent() {
			...
			for (NodeLayout predecessingNode : predecessingNodes) {
				TreeNode potentialParent = owner.layoutToTree.get(predecessingNode);
		...
		/**
		 * Checks if a potential parent would be better for this node than its current
		 * parent. A better parent has smaller depth (with exception to
		 * {@link TreeLayoutObserver#getSuperRoot() Super Root}, which has depth equal
		 * to -1 but is never a better parent than any other node).
		 *
		 * @param potentialParent potential parent to check
		 * @return true if potentialParent can be a parent of this node and is better
	...
	public static class TreeListener {
		/**
		 * Called when new node is added to the tree structure. The new node will not
		 * have any connections, so it will be a child of
		 * {@link TreeLayoutObserver#getSuperRoot() Super Root}
		 *
		...
		/**
		 * Called when a node is removed from the tree structure. The given node no
		 * longer exists in the tree at the moment of call.
		 *
		 ...
		 *
		 * @param node           node that changes its parent
		 * @param previousParent previous parent of the node
		 */
		...
		/**
		 * A convenience method that can be overridden if a listener reacts the same way
		 * to all events. By default it's called in every event handler and does
		 * nothing.
		 *
	...
	private final GraphStructureListener structureListener = new GraphStructureListener() {
		...
		public boolean nodeRemoved(LayoutContext context, NodeLayout node) {
			...
			for (Object treeListener : treeListeners) {
				TreeListener listener = (TreeListener) treeListener;
	...
	private final Map<NodeLayout, TreeNode> layoutToTree = new HashMap<>();
	...
	/**
	 * Recomputes all the information about the tree structure (the same effect as
	 * creating new <code>TreeLayoutObserver</code>).
	 */
	 ...
	 * After calling this method the information about tree structure can be updated
	 * only when {@link #recomputeTree()} is called.
	 */
	public void stop() {
	...
	/**
	 * Returns Super Root, that is an artificial node being a common parent for all
	 * nodes in observed tree structure.
	 *
	 * @return Super Root
	 */
	...
	/**
	 * Removes a listener from list of listener to be informed about changes in tree
	 * structure.
	 *
	...
	private void createTrees(NodeLayout[] nodes) {
		Set<NodeLayout> alreadyVisited = new HashSet<>();
		...
		while (!nodesToAdd.isEmpty()) {
			...
			TreeNode currentNode = factory.createTreeNode(nodeLayout, this);
			layoutToTree.put(nodeLayout, currentNode);
	...
	/**
	 * Searches for a root of a tree containing given node by continuously grabbing
	 * a predecessor of current node. If it reaches an node that exists in
	 * alreadyVisited set, it returns null. If it detects a cycle, it returns the
	 * first found node of that cycle. If it reaches a node that has no
	 * predecessors, it returns that node.
	 *
	 * @param nodeLayout     starting node
	 * @param alreadyVisited set of nodes that can't lay on path to the root (if one
	 *                       does, method stops and returns null).
	 * @return
	 */
	private static NodeLayout findRoot(NodeLayout nodeLayout, Set<NodeLayout> alreadyVisited) {
		Set<NodeLayout> alreadyVisitedRoot = new HashSet<>();
		while (true) {
