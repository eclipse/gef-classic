...
public class AlgorithmHelper {
	...
	/**
	 * Fits given entities within given bounds, preserving their relative locations.
	 *
	 * @param entities
	...
	public static void fitWithinBounds(EntityLayout[] entities, DisplayIndependentRectangle destinationBounds,
		...
		double sizeScale = Math.min(destinationBounds.width / startingBounds.width,
	...
	private static void fitSingleEntity(EntityLayout entity, DisplayIndependentRectangle destinationBounds,
			boolean resize) {
		if (entity.isMovable()) {
			entity.setLocation(destinationBounds.x + destinationBounds.width / 2,
	...
	/**
	 * Resizes the nodes so that they have a maximal area without overlapping each
	 * other, with additional empty space of 20% of node's width (or height, if
	 * bigger). It does nothing if there's less than two nodes.
	 *
	 * @param entities
	 */
