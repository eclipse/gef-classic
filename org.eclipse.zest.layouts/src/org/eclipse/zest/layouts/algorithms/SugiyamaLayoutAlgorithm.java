/*******************************************************************************
 * Copyright (c) 2012, 2024 Rene Kuhlemann and others.
 *
...
public class SugiyamaLayoutAlgorithm implements LayoutAlgorithm {
	...
	private final List<ArrayList<NodeWrapper>> layers = new ArrayList<>(MAX_LAYERS);
	...
	private void createLayers() {
		List<NodeLayout> nodes = new LinkedList<>(Arrays.asList(context.getNodes()));
	 ...
	 * chained over immediate adjacent layers down to their predecessors. This is
	 * necessary to apply the final step of the Sugiyama algorithm to refine the
	 ...
	 * top-down and bottom-up approach. It uses a heuristic approach based on the
	 * predecessor's barycenter.
	 */
	...
	private static void reduceCrossingsDown(ArrayList<NodeWrapper> layer) {
		...
		Collections.sort(layer, (node1, node2) -> (node1.index - node2.index));
		updateIndex(layer);
	}
	...
	/**
	 * Fills in virtual nodes, so the layer system finally becomes an equidistant
	 * grid
	 */
	...
	private void refineLayersDown(List<NodeWrapper> layer) {
		...
		Collections.sort(list, (node1, node2) -> (node2.getPriorityDown() - node1.getPriorityDown()));
	...
	private void refineLayersUp(List<NodeWrapper> layer) {
		// first, get a priority list
		List<NodeWrapper> list = new ArrayList<>(layer);
		Collections.sort(list, (node1, node2) -> (node2.getPriorityUp() - node1.getPriorityUp()));
		// second, remove padding from the layer's end and place them in front
		// of the current node to improve its position
		for (NodeWrapper iter : list) {
			if (iter.isPadding()) {
	...
	private void calculatePositions() {
		...
		if (dimension != null) {
			boundary = new DisplayIndependentRectangle(0, 0, dimension.preciseWidth(), dimension.preciseHeight());
		}
