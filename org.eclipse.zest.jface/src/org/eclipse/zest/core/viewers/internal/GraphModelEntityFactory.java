...
public class GraphModelEntityFactory extends AbstractStylingModelFactory {
	...
	public void refresh(Graph graph, Object element, boolean refreshLabels) {
		...
		if (refreshLabels) {
			update(node);
			for (Object element2 : node.getSourceConnections()) {
				update((GraphItem) element2);
			}
			for (Object element2 : node.getTargetConnections()) {
				update((GraphItem) element2);
			}
		}
	}
	...
	private void reconnect(Graph graph, Object element, boolean refreshLabels) {
		...
		for (Object connection : connections) {
			oldExternalConnections.add(((GraphConnection) connection).getExternalConnection());
		}
