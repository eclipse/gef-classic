 ...
 *
 * Contributors:
 *  Ed Willink - initial API and implementation from org.eclipse.ui.dialogs.ResourceListSelectionDialog
 *******************************************************************************/
...
public class ENamedElementListSelectionDialog extends SelectionDialog {
     ...
     */
    public ENamedElementListSelectionDialog(Shell parentShell, ResourceSet resourceSet,
            Class typeClass) {
        super(parentShell);
        this.resourceSet = resourceSet;
        this.typeClass = typeClass;
        setShellStyle(getShellStyle() | SWT.RESIZE);
        setTitle(typeClass.getName() + ELEMENT_SELECTION_TITLE);
    }
    ...
    private void getMatchingResources(final ArrayList resources) {
        for (TreeIterator iterator = resourceSet.getAllContents(); iterator.hasNext(); ) {
			...
			if (typeClass.isAssignableFrom(object.getClass())
		     && match(((ENamedElement)object).getName()))
    ...
    public String getParentLabel(EObject eObject) {
    	...
    	if ("".equals(parentLabel))
    		return labelProvider.getText(eContainer);
    	else
    		return parentLabel + "." + labelProvider.getText(eContainer);
    } 
...
//                ENamedElement currentResource = (ENamedElement) current.resources
//                        .get(0);
                for (int i = 1; i < descriptorsSize; i++) {
//                    ResourceDescriptor next = descriptors[i];
//                    ENamedElement nextResource = (ENamedElement) next.resources.get(0);
...
//                        currentResource = (ENamedElement) current.resources.get(0);
//                    }
                }
                ...
                if (!desc.resourcesSorted) {
                    ...
                    Collections.sort(desc.resources, new Comparator() {
                        public int compare(Object o1, Object o2) {
                            String s1 = getParentLabel((ENamedElement) o1);
                            String s2 = getParentLabel((ENamedElement) o2);
