...
public class CreateNodeCommand extends Command {
	...
	public boolean canExecute() {
		return node != null && diagram != null && loc != null
				&& (width == -1 || width > 0);
	}
	...
	public void execute() {
		if (node instanceof NamedElementView) {
			...
			if (view.getENamedElement().getName() == null) {
				// Name the classifier or package if it doesn't have a name;
				// it'll have a
				// name if it's being dragged from the outline
				view.getENamedElement().setName(
						"DefaultName" + (int) (Math.random() * 10000000));
			}
			if (view.getENamedElement() instanceof EClassifier) {
				// Give the classifier a default package if it doesn't belong to
				// one
				if (((EClassifier) view.getENamedElement()).getEPackage() == null) {
					((EPackage) diagram.getImports().get(0)).getEClassifiers()
							.add((EClassifier) view.getENamedElement());
					packageSet = true;
				}
			} else if (view.getENamedElement() instanceof EPackage) {
				if (!Utilities.importsPackage(
						(EPackage) view.getENamedElement(), diagram)) {
					// If this is a new package, add it to an ecore file and to
					// the diagram's
					// imports
					((EPackage) diagram.getImports().get(0)).eResource()
							.getContents().add(view.getENamedElement());
	...
	public void undo() {
		...
		if (node instanceof NamedElementView) {
			NamedElementView view = (NamedElementView) node;
			if (packageSet)
				((EClassifier) view.getENamedElement()).getEPackage()
						.getEClassifiers().remove(view.getENamedElement());
