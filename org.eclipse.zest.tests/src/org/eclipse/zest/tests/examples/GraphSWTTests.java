...
public class GraphSWTTests extends AbstractGraphTest {
	...
	public void testCustomLayout() {
		...
		assertConnection(0, "Paper", "", "Rock");
		assertConnection(1, "Rock", "", "Scissors");
		assertConnection(2, "Scissors", "", "Paper");
	...
	public void testGraphSnippet2() {
		assertNode(graph, 0, "Information");
		assertNode(graph, 1, "Warning");
		assertNode(graph, 2, "Error");
		assertEquals(graph.getNodes().size(), 3);

		assertConnection(0, "Information", "", "Warning");
		assertConnection(1, "Warning", "", "Error");
		assertConnection(2, "Error", "", "Error");
		assertEquals(graph.getConnections().size(), 3);
	...
	public void testGraphSnippet5() {
		assertNode(graph, 0, "org.eclipse.Information");
		assertNode(graph, 1, "org.eclipse.Warning");
		assertNode(graph, 2, "org.eclipse.Error");
		assertEquals(graph.getNodes().size(), 3);
	...
	public void testGraphSnippet6() {
		...
		for (int i = 0; i < labels.size(); ++i) {
			...
			assertFishEyeFigure(location.x + 1, location.y + 1, labels.get(i));
		}
	}
	...
	public void testGraphSnippet9() {
		assertNode(graph, 0, "Root");
		assertEquals(graph.getNodes().size(), 1);

		assertConnection(0, "Root", "A to A", "Root");
	...
	public void testGraphSnippet10() {
		...
		assertConnection(0, "Paper", 0 /* ° */, "Rock");
		// Old connection is removed and a new one is added
		robot.select(button);
		assertConnection(2, "Paper", 11 /* ° */, "Rock");
		robot.select(button);
		assertConnection(2, "Paper", 21 /* ° */, "Rock");
		robot.select(button);
		assertConnection(2, "Paper", 31 /* ° */, "Rock");
	}
	...
	public void testGraphSnippet11() {
		...
		assertNode(graph, 0, "Node 1");
		assertNode(graph, 1, "Node 2");
		assertEquals(graph.getNodes().size(), 2);

		assertConnection(0, "Node 1", 13 /* ° */, "Node 2");
		assertConnection(1, "Node 1", 13 /* ° */, "Node 2");
		assertConnection(2, "Node 1", 25 /* ° */, "Node 2");
		assertConnection(3, "Node 1", 25 /* ° */, "Node 2");
		assertConnection(4, "Node 1", 35 /* ° */, "Node 2");
		assertConnection(5, "Node 1", 35 /* ° */, "Node 2");
		assertConnection(6, "Node 1", 0 /* ° */, "Node 2");
	...
	public void testGraphSnippet12() {
		...
		assertNode(graph, 2, "PDE");
		assertNode(graph, 3, "Zest");
		assertNode(graph, 4, "PDE Viz tool");
	...
	public void testGraphSnippet13() throws Throwable {
		assertNode(graph, 0, "Canada");
		assertNode(graph, 1, "USA");
		assertEquals(graph.getNodes().size(), 2);
		assertEquals(graph.getConnections().size(), 2);
		...
		assertNode(container, 0, "Chris A.");
	...
	public void testHelloWorld() {
		assertNode(graph, 0, "Hello");
		assertNode(graph, 1, "World");
		assertEquals(graph.getNodes().size(), 2);

		assertConnection(0, "Hello", "", "World");
		assertEquals(graph.getConnections().size(), 1);
	}
	...
	public void testNestedGraphSnippet() {
		...
		assertNode(container, 0, "SomeClass.java");
	...
	public void testNestedGraphSnippet2() {
		assertNode(graph, 0, "Machine 1 (prop:1)");
		assertNode(graph, 1, "Machine 2");
		assertNode(graph, 2, "Machine 3");
		assertEquals(graph.getNodes().size(), 3);
		...
		assertNode(container1, 0, "Host 1");
		assertNode(container1, 1, "Host 2");
		...
		assertNode(container2, 0, "JSP Object 3");
	...
	public void testPaintSnippet() {
		...
		// The screenshot has a 10-pixel-wide border
		int offsetX = 10;
		int offsetY = 10;
		...
		try {
			...
			int lightBlueInt = getIntValue(graph.LIGHT_BLUE);
			for (GraphNode node : graph.getNodes()) {
				Point location = node.getLocation();
				// The edges of the figure are round, so we need to make sure that the pixel is
				// really part of the background.
				int x = location.x + offsetX + 10;
				int y = location.y + offsetY + 2;
				assertEquals(imageData.getPixel(x, y), lightBlueInt);
			}
		} finally {
			gc.dispose();
			image.dispose();
	...
	public void testZoomSnippet() {
		...
		assertFishEyeFigure(location.x, location.y, "SomeClass.java");
	}
}
