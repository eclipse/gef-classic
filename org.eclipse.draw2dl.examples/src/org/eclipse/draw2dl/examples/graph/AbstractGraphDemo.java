 ...
 *******************************************************************************/
package org.eclipse.draw2dl.examples.graph;
...
public abstract class AbstractGraphDemo {
	...
	protected org.eclipse.draw2dl.IFigure contents;
	/** Name of graph test method to run */
	protected static String graphMethod;
	/** direction to use for the graph */
	protected static int graphDirection = org.eclipse.draw2dl.PositionConstants.SOUTH;
	/** Demo shell */
	protected Shell shell;

	private org.eclipse.draw2dl.FigureCanvas fc;

	static class TopOrBottomAnchor extends org.eclipse.draw2dl.ChopboxAnchor {
		public TopOrBottomAnchor(org.eclipse.draw2dl.IFigure owner) {
			super(owner);
		}

		public org.eclipse.draw2dl.geometry.Point getLocation(org.eclipse.draw2dl.geometry.Point reference) {
			org.eclipse.draw2dl.geometry.Point p;
	...
	static class LeftOrRightAnchor extends ChopboxAnchor {
		public LeftOrRightAnchor(org.eclipse.draw2dl.IFigure owner) {
			super(owner);
		}
	 ...
	 *            the edge
	 */
	static void buildEdgeFigure(org.eclipse.draw2dl.Figure contents, Edge edge) {
		org.eclipse.draw2dl.PolylineConnection conn = connection(edge);
		conn.setForegroundColor(org.eclipse.draw2dl.ColorConstants.gray);
		org.eclipse.draw2dl.PolygonDecoration dec = new PolygonDecoration();
	...
	static void buildNodeFigure(Figure contents, Node node) {
		org.eclipse.draw2dl.Label label;
		label = new Label();
	 ...
	 */
	static org.eclipse.draw2dl.PolylineConnection connection(Edge e) {
		org.eclipse.draw2dl.PolylineConnection conn = new PolylineConnection();
		...
		if (nodes != null) {
			for (int i = 0; i < nodes.size(); i++) {
				...
				int x = n.x;
				int y = n.y;
				bends.add(new org.eclipse.draw2dl.AbsoluteBendpoint(x, y));
	 ...
	 */
	protected org.eclipse.draw2dl.FigureCanvas getFigureCanvas() {
		return fc;
	}
	...
	protected void hookShell() {
		...
		directionCombo.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				switch (index) {
				case 0:
					graphDirection = org.eclipse.draw2dl.PositionConstants.SOUTH;
					break;
				case 1:
	...
	protected void run() {
		...
		setFigureCanvas(new org.eclipse.draw2dl.FigureCanvas(shell));
