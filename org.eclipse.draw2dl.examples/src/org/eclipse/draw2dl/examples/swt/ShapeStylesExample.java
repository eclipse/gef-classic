 ...
 *******************************************************************************/
package org.eclipse.draw2dl.examples.swt;
...
import org.eclipse.draw2dl.RoundedRectangle;
	...
	static StyleAxis[] styleAxes = new StyleAxis[] {
		new StyleAxis(
			...
			new StyleItem[] {
				...
				public void applyTo(org.eclipse.draw2dl.Shape shape, int i) {
	...
	static void refreshGrid() {
		...
		for(int y=1; y < yAxis.getCount() + 1; y++) {
			...
			for(int x=1; x < xAxis.getCount() + 1; x++) {
				...
				org.eclipse.draw2dl.Shape shape;
				if(sampleShape == Polyline.class) {
					...
					try {
						shape = (org.eclipse.draw2dl.Shape)sampleShape.getConstructor(null).newInstance(null);
					} catch (Exception e) {
