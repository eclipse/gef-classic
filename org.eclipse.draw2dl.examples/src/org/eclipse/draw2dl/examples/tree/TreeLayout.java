 ...
 */
public class TreeLayout extends org.eclipse.draw2dl.AbstractLayout {
...
public void layout(org.eclipse.draw2dl.IFigure container) {
	...
	org.eclipse.draw2dl.geometry.Point reference = transposer.t(container.getBounds().getLocation());
	org.eclipse.draw2dl.geometry.Point currentXY = reference.getCopy();
	...
	for (int i = 0; i < subtrees.size(); i++) {
		...
		if (shiftRight > 0) {
			...
			org.eclipse.draw2dl.geometry.Point correction = transposer.t(new org.eclipse.draw2dl.geometry.Point(shiftRight, 0));
			for (int j=0; j<=i; j++)
		...
		if (contactDepth > previousSubtreeDepth) {
			...
			for (int j = begin + 1; j < end; j++) {
				...
				org.eclipse.draw2dl.geometry.Point shift =
