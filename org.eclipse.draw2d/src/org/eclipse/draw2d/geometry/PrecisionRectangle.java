...
public final class PrecisionRectangle extends Rectangle {
	...
	public Rectangle union(Rectangle other) {
		double newright = Math.max(preciseRight(),
				other.preciseX() + other.preciseWidth());
		double newbottom = Math.max(preciseBottom(),
				other.preciseY() + other.preciseHeight());
		preciseX = Math.min(preciseX, other.preciseX());
		preciseY = Math.min(preciseY, other.preciseY());
		preciseWidth = newright - preciseX;
		preciseHeight = newbottom - preciseY;
	...
	/**
	 * Updates the integer values based on the current precise values. The
	 * integer values ar the floor of the double values. This is called
	 * automatically when calling api which is overridden in this class.
	 * 
	 * @since 3.0
	 */
	...
	public boolean contains(Point p) {
		return preciseX <= p.preciseX()
				&& p.preciseX() <= preciseX + preciseWidth
				&& preciseY <= p.preciseY()
				&& p.preciseY() <= preciseY + preciseHeight;
	}
