 ...
 * {@link org.eclipse.draw2dl.text.ContentBox fragments} know of the LineRoot
 * they belong to.
 * 
 * @author Randy Hudson
 * @author Pratik Shah
 * @since 3.1
 */
public class LineRoot extends org.eclipse.draw2dl.text.LineBox {
	...
	/**
	 * @see CompositeBox#add(org.eclipse.draw2dl.text.FlowBox)
	 */
	public void add(org.eclipse.draw2dl.text.FlowBox child) {
		super.add(child);
	...
	private void bidiCommit() {
		...
		while (i >= 0 && i < result.size()) {
			org.eclipse.draw2dl.text.FlowBox box = (org.eclipse.draw2dl.text.FlowBox) result.get(i);
			box.setX(xLocation);
	...
	private void buildBidiTree(org.eclipse.draw2dl.text.FlowBox box, BidiLevelNode node, List branches) {
		if (box instanceof org.eclipse.draw2dl.text.LineBox) {
			List children = ((org.eclipse.draw2dl.text.LineBox) box).getFragments();
			for (int i = 0; i < children.size(); i++)
				buildBidiTree((org.eclipse.draw2dl.text.FlowBox) children.get(i), node, branches);
			if (box != this)
				branches.add(box);
		} else {
			org.eclipse.draw2dl.text.ContentBox leafBox = (ContentBox) box;
	 ...
	 * they're contained.
	 */
	private void contiguousCommit(org.eclipse.draw2dl.text.FlowBox box, int x) {
		box.setX(x);
		if (box instanceof org.eclipse.draw2dl.text.LineBox) {
			List fragments = ((org.eclipse.draw2dl.text.LineBox) box).getFragments();
			int i = isMirrored ? fragments.size() - 1 : 0;
			while (i >= 0 && i < fragments.size()) {
				org.eclipse.draw2dl.text.FlowBox child = (org.eclipse.draw2dl.text.FlowBox) fragments.get(i);
	...
	private Result findParent(org.eclipse.draw2dl.text.NestedLine line, List branches, int afterIndex) {
		for (int i = afterIndex + 1; i < branches.size(); i++) {
			org.eclipse.draw2dl.text.NestedLine box = (org.eclipse.draw2dl.text.NestedLine) branches.get(i);
	...
	private void layoutNestedLines(List branches) {
		for (int i = 0; i < branches.size(); i++) {
			org.eclipse.draw2dl.text.NestedLine parent = (org.eclipse.draw2dl.text.NestedLine) branches.get(i);
			org.eclipse.draw2dl.text.FlowBox prevChild = null;
			Rectangle bounds = null;
			List frags = parent.getFragments();
			for (int j = 0; j < frags.size(); j++) {
				org.eclipse.draw2dl.text.FlowBox child = (FlowBox) frags.get(j);
				if (prevChild != null
					...
					org.eclipse.draw2dl.text.NestedLine newBox = new NestedLine(parentFig);
	...
	private static class Result {
		private int index;
		private org.eclipse.draw2dl.text.LineBox parent;
