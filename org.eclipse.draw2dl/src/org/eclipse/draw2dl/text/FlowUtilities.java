...
public class FlowUtilities {
	 ...
	 */
	protected float getAverageCharWidth(org.eclipse.draw2dl.text.TextFragmentBox fragment, Font font) {
	...
	static int getBorderAscent(org.eclipse.draw2dl.text.InlineFlow owner) {
		if (owner.getBorder() instanceof org.eclipse.draw2dl.text.FlowBorder) {
			org.eclipse.draw2dl.text.FlowBorder border = (org.eclipse.draw2dl.text.FlowBorder) owner.getBorder();
			return border.getInsets(owner).top;
		}
		return 0;
	}

	static int getBorderAscentWithMargin(org.eclipse.draw2dl.text.InlineFlow owner) {
		if (owner.getBorder() instanceof org.eclipse.draw2dl.text.FlowBorder) {
			org.eclipse.draw2dl.text.FlowBorder border = (org.eclipse.draw2dl.text.FlowBorder) owner.getBorder();
	...
	static int getBorderDescent(org.eclipse.draw2dl.text.InlineFlow owner) {
		if (owner.getBorder() instanceof org.eclipse.draw2dl.text.FlowBorder) {
			org.eclipse.draw2dl.text.FlowBorder border = (org.eclipse.draw2dl.text.FlowBorder) owner.getBorder();
	...
	static int getBorderDescentWithMargin(InlineFlow owner) {
		if (owner.getBorder() instanceof org.eclipse.draw2dl.text.FlowBorder) {
			org.eclipse.draw2dl.text.FlowBorder border = (FlowBorder) owner.getBorder();
	 ...
	 * @since 3.1
	 */
	private static void initBidi(org.eclipse.draw2dl.text.TextFragmentBox frag, String string, Font font) {
	...
	private int measureString(org.eclipse.draw2dl.text.TextFragmentBox frag, String string, int guess,
                              Font font) {
		if (frag.requiresBidi()) {
	 ...
	 */
	final protected void setupFragment(org.eclipse.draw2dl.text.TextFragmentBox fragment, Font font,
                                       String string) {
	...
	final protected int wrapFragmentInContext(TextFragmentBox frag,
		...
		int absoluteMin = 0;
		int max, min = 1;
		if (wrapping == org.eclipse.draw2dl.text.ParagraphTextLayout.WORD_WRAP_HARD) {
