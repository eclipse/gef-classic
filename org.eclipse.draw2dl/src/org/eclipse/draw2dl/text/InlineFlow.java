 ...
 * @since 2.0
 */
public class InlineFlow extends org.eclipse.draw2dl.text.FlowFigure {
	...
	public boolean containsPoint(int x, int y) {
		if (super.containsPoint(x, y)) {
			...
			for (int i = 0; i < frags.size(); i++)
				if (((org.eclipse.draw2dl.text.FlowBox) frags.get(i)).containsPoint(x, y))
					return true;
		}
	...
	/**
	 * Overridden to paint a {@link org.eclipse.draw2dl.text.FlowBorder} if present, and draw selection.
	...
	protected void paintBorder(Graphics graphics) {
		if (getBorder() != null) {
			org.eclipse.draw2dl.text.FlowBorder fb = (org.eclipse.draw2dl.text.FlowBorder) getBorder();
			List frags = getFragments();
			Rectangle where = new Rectangle();
			int sides;
			for (int i = 0; i < frags.size(); i++) {
				org.eclipse.draw2dl.text.FlowBox box = (org.eclipse.draw2dl.text.FlowBox) frags.get(i);
	...
	protected void paintSelection(Graphics graphics) {
		...
		org.eclipse.draw2dl.text.FlowBox box;
		for (int i = 0; i < list.size(); i++) {
			box = (org.eclipse.draw2dl.text.FlowBox) list.get(i);
	...
	/**
	 * Overridden to assert that only {@link org.eclipse.draw2dl.text.FlowBorder} is used.
