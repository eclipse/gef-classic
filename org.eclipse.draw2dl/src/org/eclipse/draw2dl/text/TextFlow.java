 ...
 * @since 2.1
 */
public class TextFlow extends org.eclipse.draw2dl.text.InlineFlow {

	static final String ELLIPSIS = "..."; //$NON-NLS-1$
	private org.eclipse.draw2dl.text.BidiInfo bidiInfo;
	...
	boolean addLeadingWordWidth(String text, int[] width) {
		...
		text = 'a' + text + 'a';
		org.eclipse.draw2dl.text.FlowUtilities.LINE_BREAK.setText(text);
		int index = org.eclipse.draw2dl.text.FlowUtilities.LINE_BREAK.next() - 1;
		if (index == 0)
			return true;
		...
		if (bidiInfo == null)
			...
			TextLayout textLayout = org.eclipse.draw2dl.text.FlowUtilities.getTextLayout();
	 ...
	 * 
	 * @see FlowFigure#contributeBidi(org.eclipse.draw2dl.text.BidiProcessor)
	 */
	...
	/**
	 * @see org.eclipse.draw2dl.text.InlineFlow#createDefaultFlowLayout()
	 */
	...
	private int findNextLineOffset(Point p, int[] trailing) {
		...
		org.eclipse.draw2dl.text.TextFragmentBox closestBox = null;
		int index = 0;
		List fragments = getFragmentsWithoutBorder();
		for (int i = fragments.size() - 1; i >= 0; i--) {
			org.eclipse.draw2dl.text.TextFragmentBox box = (org.eclipse.draw2dl.text.TextFragmentBox) fragments.get(i);
	...
	private int findOffset(Point p, int[] trailing, org.eclipse.draw2dl.text.TextFragmentBox box,
			int boxIndex) {
		if (box == null)
			return -1;
		TextLayout layout = org.eclipse.draw2dl.text.FlowUtilities.getTextLayout();
	 ...
	 * @since 3.1
	 */
	public org.eclipse.draw2dl.text.BidiInfo getBidiInfo() {
		return bidiInfo;
	}

	private int getBidiPrefixLength(org.eclipse.draw2dl.text.TextFragmentBox box, int index) {
	 ...
	 * @since 3.1
	 */
	protected String getBidiSubstring(org.eclipse.draw2dl.text.TextFragmentBox box, int index) {
		if (box.getBidiLevel() < 1)
			return getText().substring(box.offset, box.offset + box.length);

		StringBuffer buffer = new StringBuffer(box.length + 3);
		buffer.append(box.isRightToLeft() ? org.eclipse.draw2dl.text.BidiChars.RLO : org.eclipse.draw2dl.text.BidiChars.LRO);
		if (index == 0 && bidiInfo.leadingJoiner)
			buffer.append(org.eclipse.draw2dl.text.BidiChars.ZWJ);
	 ...
	 */
	public org.eclipse.draw2dl.text.CaretInfo getCaretPlacement(int offset, boolean trailing) {
		...
		org.eclipse.draw2dl.text.TextFragmentBox box;
		do
			box = (org.eclipse.draw2dl.text.TextFragmentBox) fragments.get(--i);
		...
		org.eclipse.draw2dl.text.CaretInfo info = new CaretInfo(where.x, where.y, box.getAscent(),
	...
	Point getPointInBox(org.eclipse.draw2dl.text.TextFragmentBox box, int offset, int index,
                        boolean trailing) {
		offset -= box.offset;
	...
	public int getOffset(Point p, int trailing[], Dimension proximity) {
		...
		if (getBorder() instanceof org.eclipse.draw2dl.text.FlowBorder) {
			i++;
			size--;
		}
		for (; i < size; i++) {
	...
	int getVisibleDescent() {
		if (getBorder() instanceof org.eclipse.draw2dl.text.FlowBorder) {
			org.eclipse.draw2dl.text.FlowBorder border = (FlowBorder) getBorder();
			return border.getInsets(this).bottom + getDescent();
		}
		return getDescent();
	}

	private int hDistanceBetween(org.eclipse.draw2dl.text.TextFragmentBox box, int x) {
		if (x < box.getX())
			return box.getX() - x;
	...
	public boolean isTextTruncated() {
		for (int i = 0; i < fragments.size(); i++) {
			if (((org.eclipse.draw2dl.text.TextFragmentBox) fragments.get(i)).isTruncated())
				return true;
		}
		return false;
	}
	...
	protected void paintFigure(Graphics g) {
		org.eclipse.draw2dl.text.TextFragmentBox frag;
		g.getClip(Rectangle.SINGLETON);
		int yStart = Rectangle.SINGLETON.y;
		int yEnd = Rectangle.SINGLETON.bottom();

		for (int i = 0; i < fragments.size(); i++) {
			frag = (org.eclipse.draw2dl.text.TextFragmentBox) fragments.get(i);
	...
	protected void paintText(Graphics g, String draw, int x, int y,
		...
		if (bidiLevel == -1) {
			...
			TextLayout tl = org.eclipse.draw2dl.text.FlowUtilities.getTextLayout();
			if (isMirrored())
	...
	/**
	 * @see FlowFigure#setBidiInfo(org.eclipse.draw2dl.text.BidiInfo)
	 */
	 ...
	 * @since 3.4
	 */
	protected org.eclipse.draw2dl.text.FlowUtilities getFlowUtilities() {
