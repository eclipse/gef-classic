...
public abstract class LineBox extends CompositeBox {
	...
	public int getAscent() {
		...
		for (int i = 0; i < fragments.size(); i++)
			ascent = Math.max(ascent, ((org.eclipse.draw2dl.text.FlowBox) fragments.get(i)).getAscent());
		return ascent;
	}
	...
	/**
	 * @see org.eclipse.draw2dl.text.FlowBox#getDescent()
	 */
	public int getDescent() {
		int descent = 0;
		for (int i = 0; i < fragments.size(); i++)
			descent = Math.max(descent,
					((org.eclipse.draw2dl.text.FlowBox) fragments.get(i)).getDescent());
		return descent;
	}
	...
	/**
	 * @see org.eclipse.draw2dl.text.FlowBox#requiresBidi()
	 */
	public boolean requiresBidi() {
		for (Iterator iter = getFragments().iterator(); iter.hasNext();) {
			org.eclipse.draw2dl.text.FlowBox box = (FlowBox) iter.next();
			if (box.requiresBidi())
				return true;
		}
		return false;
