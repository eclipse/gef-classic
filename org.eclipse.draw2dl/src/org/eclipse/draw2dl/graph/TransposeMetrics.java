...
class TransposeMetrics extends org.eclipse.draw2dl.graph.GraphVisitor {
	...
	public void revisit(DirectedGraph g) {
		...
		g.setDefaultPadding(t.t(g.getDefaultPadding()));
		for (int i = 0; i < g.nodes.size(); i++) {
			org.eclipse.draw2dl.graph.Node node = (Node) g.nodes.get(i);
			temp = node.width;
			node.width = node.height;
			node.height = temp;
			temp = node.y;
		...
		for (int i = 0; i < g.edges.size(); i++) {
			...
			for (int b = 0; b < bends.size(); b++) {
				org.eclipse.draw2dl.graph.VirtualNode vnode = (VirtualNode) bends.get(b);
				temp = vnode.y;
				vnode.y = vnode.x;
				vnode.x = temp;
				temp = vnode.width;
