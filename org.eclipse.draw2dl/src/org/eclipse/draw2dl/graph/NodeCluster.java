...
class NodeCluster extends NodeList {
	...
	org.eclipse.draw2dl.graph.CollapsedEdges leftLinks[] = new org.eclipse.draw2dl.graph.CollapsedEdges[10];
	org.eclipse.draw2dl.graph.CollapsedEdges rightLinks[] = new org.eclipse.draw2dl.graph.CollapsedEdges[10];
	...
	void addLeftNeighbor(NodeCluster neighbor, org.eclipse.draw2dl.graph.CollapsedEdges link) {
		...
		if (leftNeighbors.length == leftCount) {
			...
			org.eclipse.draw2dl.graph.CollapsedEdges newLinks[] = new org.eclipse.draw2dl.graph.CollapsedEdges[newSize];
	...
	void addRightNeighbor(NodeCluster neighbor, org.eclipse.draw2dl.graph.CollapsedEdges link) {
	...
	public void adjustRank(int delta, Collection affected) {
		...
		org.eclipse.draw2dl.graph.CollapsedEdges edges;
	...
	org.eclipse.draw2dl.graph.CollapsedEdges getLeftNeighbor(NodeCluster neighbor) {
		for (int i = 0; i < leftCount; i++) {
	...
	org.eclipse.draw2dl.graph.CollapsedEdges getRightNeighbor(NodeCluster neighbor) {
		for (int i = 0; i < rightCount; i++) {
	...
	void initValues() {
		...
		for (int i = 0; i < leftCount; i++) {
			org.eclipse.draw2dl.graph.CollapsedEdges edges = leftLinks[i];
		...
		for (int i = 0; i < rightCount; i++) {
			org.eclipse.draw2dl.graph.CollapsedEdges edges = rightLinks[i];
			weightedTotal += edges.getWeightedPull();
			unweightedDivisor += edges.collapsedCount;
			unweightedTotal += edges.tightestEdge.getSlack();
			weightedDivisor += edges.collapsedWeight;
