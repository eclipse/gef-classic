...
class NestingTree {
	...
	int size;
	double sortValue;
	org.eclipse.draw2dl.graph.Node subgraph;

	private static void addToNestingTree(Map map, org.eclipse.draw2dl.graph.Node child) {
		org.eclipse.draw2dl.graph.Subgraph subgraph = child.getParent();
	...
	static NestingTree buildNestingTreeForRank(org.eclipse.draw2dl.graph.Rank rank) {
	...
	void calculateSortValues() {
		...
		for (int i = 0; i < contents.size(); i++) {
			...
			if (o instanceof NestingTree) {
				...
				org.eclipse.draw2dl.graph.Node n = (org.eclipse.draw2dl.graph.Node) o;
				n.sortValue = n.index;
				total += n.index;
				size++;
			}
	...
	void repopulateRank(Rank r) {
		for (int i = 0; i < contents.size(); i++) {
			Object o = contents.get(i);
			if (o instanceof org.eclipse.draw2dl.graph.Node)
				r.add(o);
			else
	...
	boolean swap(int index) {
		...
		double iL = (left instanceof org.eclipse.draw2dl.graph.Node) ? ((org.eclipse.draw2dl.graph.Node) left).sortValue
				: ((NestingTree) left).sortValue;
		double iR = (right instanceof org.eclipse.draw2dl.graph.Node) ? ((Node) right).sortValue
