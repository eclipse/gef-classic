 ...
 * @since 3.1
 */
class VirtualNodeCreation extends org.eclipse.draw2dl.graph.RevertableChange {

	private final org.eclipse.draw2dl.graph.Edge edge;
	private final org.eclipse.draw2dl.graph.DirectedGraph graph;
	private org.eclipse.draw2dl.graph.Node nodes[];
	private org.eclipse.draw2dl.graph.Edge[] edges;
	 ...
	 */
	public VirtualNodeCreation(org.eclipse.draw2dl.graph.Edge edge, DirectedGraph graph) {
		this.edge = edge;
		this.graph = graph;
		...
		org.eclipse.draw2dl.graph.Node prevNode = edge.source;
		org.eclipse.draw2dl.graph.Node currentNode;
		org.eclipse.draw2dl.graph.Edge currentEdge;
		nodes = new Node[size];
		edges = new org.eclipse.draw2dl.graph.Edge[size + 1];

		Insets padding = new Insets(0, edge.padding, 0, edge.padding);

		Subgraph s = org.eclipse.draw2dl.graph.GraphUtilities.getCommonAncestor(edge.source, edge.target);
		...
		for (int i = 0; i < size; i++) {
			...
			currentEdge = new org.eclipse.draw2dl.graph.Edge(prevNode, currentNode, 1, edge.weight
					* LONG_EDGE_X);
			if (i == 0) {
