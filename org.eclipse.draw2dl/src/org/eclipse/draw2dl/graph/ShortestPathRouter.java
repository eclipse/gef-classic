...
/**
 * Bends a collection of {@link org.eclipse.draw2dl.graph.Path Paths} around rectangular obstacles. This
...
public class ShortestPathRouter {
	...
	static class PathStack extends ArrayList {
		...
		org.eclipse.draw2dl.graph.Path pop() {
			return (org.eclipse.draw2dl.graph.Path) remove(size() - 1);
		}

		void push(org.eclipse.draw2dl.graph.Path path) {
			add(path);
		}
	}
	...
	public boolean addObstacle(Rectangle rect) {
		return internalAddObstacle(new org.eclipse.draw2dl.graph.Obstacle(rect, this));
	}
	 ...
	 *            the path to add.
	 */
	public void addPath(org.eclipse.draw2dl.graph.Path path) {
		userPaths.add(path);
	...
	private void bendPaths() {
		for (int i = 0; i < orderedPaths.size(); i++) {
			org.eclipse.draw2dl.graph.Path path = (org.eclipse.draw2dl.graph.Path) orderedPaths.get(i);
			org.eclipse.draw2dl.graph.Segment segment = null;
			path.points.addPoint(new Point(path.start.x, path.start.y));
			for (int v = 0; v < path.grownSegments.size(); v++) {
				segment = (org.eclipse.draw2dl.graph.Segment) path.grownSegments.get(v);
				org.eclipse.draw2dl.graph.Vertex vertex = segment.end;
	 ...
	 *            the vertex to check
	 */
	private void checkVertexForIntersections(org.eclipse.draw2dl.graph.Vertex vertex) {
		...
		for (int o = 0; o < userObstacles.size(); o++) {
			org.eclipse.draw2dl.graph.Obstacle obs = (org.eclipse.draw2dl.graph.Obstacle) userObstacles.get(o);
	...
	private void checkVertexIntersections() {
		for (int i = 0; i < workingPaths.size(); i++) {
			org.eclipse.draw2dl.graph.Path path = (org.eclipse.draw2dl.graph.Path) workingPaths.get(i);

			for (int s = 0; s < path.segments.size() - 1; s++) {
				org.eclipse.draw2dl.graph.Vertex vertex = ((org.eclipse.draw2dl.graph.Segment) path.segments.get(s)).end;
	...
	private void countVertices() {
		for (int i = 0; i < workingPaths.size(); i++) {
			...
			for (int v = 0; v < path.segments.size() - 1; v++)
				((org.eclipse.draw2dl.graph.Segment) path.segments.get(v)).end.totalCount++;
		}
	}
	 ...
	 */
	private boolean dirtyPathsOn(org.eclipse.draw2dl.graph.Vertex vertex) {
		List paths = vertex.paths;
		if (paths != null && paths.size() != 0) {
			for (int i = 0; i < paths.size(); i++)
				((org.eclipse.draw2dl.graph.Path) paths.get(i)).isDirty = true;
			return true;
		}
		return false;
	}
	 ...
	 */
	private org.eclipse.draw2dl.graph.Vertex getNearestVertex(org.eclipse.draw2dl.graph.Vertex v1, org.eclipse.draw2dl.graph.Vertex v2, org.eclipse.draw2dl.graph.Segment segment) {
	 ...
	 */
	private org.eclipse.draw2dl.graph.Path getSubpathForSplit(org.eclipse.draw2dl.graph.Path path, org.eclipse.draw2dl.graph.Segment segment) {
		org.eclipse.draw2dl.graph.Path newPath = path.getSubPath(segment);
	...
	private void growObstaclesPass() {
		...
		for (int i = 0; i < userObstacles.size(); i++)
			((org.eclipse.draw2dl.graph.Obstacle) userObstacles.get(i)).growVertices();
		...
		for (int i = 0; i < workingPaths.size(); i++) {
			...
			for (int e = 0; e < path.excludedObstacles.size(); e++)
				((org.eclipse.draw2dl.graph.Obstacle) path.excludedObstacles.get(e)).exclude = true;

			if (path.grownSegments.size() == 0) {
				for (int s = 0; s < path.segments.size(); s++)
					testOffsetSegmentForIntersections(
							(org.eclipse.draw2dl.graph.Segment) path.segments.get(s), -1, path);
			} else {
				int counter = 0;
				...
				for (int s = 0; s < currentSegments.size(); s++)
					counter += testOffsetSegmentForIntersections(
							(org.eclipse.draw2dl.graph.Segment) currentSegments.get(s), s + counter, path);
			}

			for (int e = 0; e < path.excludedObstacles.size(); e++)
				((org.eclipse.draw2dl.graph.Obstacle) path.excludedObstacles.get(e)).exclude = false;

		}

		// revert obstacles
		for (int i = 0; i < userObstacles.size(); i++)
			((org.eclipse.draw2dl.graph.Obstacle) userObstacles.get(i)).shrinkVertices();
	}
	 ...
	 *            the obstacle
	 */
	private boolean internalAddObstacle(org.eclipse.draw2dl.graph.Obstacle obs) {
	...
	private boolean internalRemoveObstacle(Rectangle rect) {
		org.eclipse.draw2dl.graph.Obstacle obs = null;
		int index = -1;
		for (int i = 0; i < userObstacles.size(); i++) {
			obs = (org.eclipse.draw2dl.graph.Obstacle) userObstacles.get(i);
			if (obs.equals(rect)) {
				index = i;
				break;
			}
		...
		for (int p = 0; p < workingPaths.size(); p++) {
			org.eclipse.draw2dl.graph.Path path = (org.eclipse.draw2dl.graph.Path) workingPaths.get(p);
			if (path.isDirty)
				continue;
	 ...
	 *            the path
	 */
	private void labelPath(org.eclipse.draw2dl.graph.Path path) {
		org.eclipse.draw2dl.graph.Segment segment = null;
		org.eclipse.draw2dl.graph.Segment nextSegment = null;
		org.eclipse.draw2dl.graph.Vertex vertex = null;
		boolean agree = false;
		for (int v = 0; v < path.grownSegments.size() - 1; v++) {
			segment = (org.eclipse.draw2dl.graph.Segment) path.grownSegments.get(v);
			nextSegment = (org.eclipse.draw2dl.graph.Segment) path.grownSegments.get(v + 1);
			vertex = segment.end;
			long crossProduct = segment.crossProduct(new org.eclipse.draw2dl.graph.Segment(vertex,
					vertex.obs.center));

			if (vertex.type == org.eclipse.draw2dl.graph.Vertex.NOT_SET) {
				labelVertex(segment, crossProduct, path);
			} else if (!path.isInverted
					&& ((crossProduct > 0 && vertex.type == org.eclipse.draw2dl.graph.Vertex.OUTIE) || (crossProduct < 0 && vertex.type == org.eclipse.draw2dl.graph.Vertex.INNIE))) {
				if (agree) {
					// split detected.
			...
			if (vertex.paths != null) {
				for (int i = 0; i < vertex.paths.size(); i++) {
					org.eclipse.draw2dl.graph.Path nextPath = (org.eclipse.draw2dl.graph.Path) vertex.paths.get(i);
	...
	private void labelPaths() {
		org.eclipse.draw2dl.graph.Path path = null;
		for (int i = 0; i < workingPaths.size(); i++) {
			path = (org.eclipse.draw2dl.graph.Path) workingPaths.get(i);
			stack.push(path);
		}
	 ...
	 *            the path
	 */
	private void labelVertex(org.eclipse.draw2dl.graph.Segment segment, long crossProduct, org.eclipse.draw2dl.graph.Path path) {
		// assumes vertex in question is segment.end
		if (crossProduct > 0) {
			if (path.isInverted)
				segment.end.type = org.eclipse.draw2dl.graph.Vertex.OUTIE;
			else
				segment.end.type = org.eclipse.draw2dl.graph.Vertex.INNIE;
		} else if (crossProduct < 0) {
			if (path.isInverted)
				segment.end.type = org.eclipse.draw2dl.graph.Vertex.INNIE;
			else
				segment.end.type = org.eclipse.draw2dl.graph.Vertex.OUTIE;
		} else if (segment.start.type != org.eclipse.draw2dl.graph.Vertex.NOT_SET)
	 ...
	 *            the path
	 */
	private void orderPath(org.eclipse.draw2dl.graph.Path path) {
		if (path.isMarked)
			return;
		path.isMarked = true;
		org.eclipse.draw2dl.graph.Segment segment = null;
		...
		for (int v = 0; v < path.grownSegments.size() - 1; v++) {
			...
			for (int i = 0; i < vertex.paths.size(); i++) {
				org.eclipse.draw2dl.graph.Path vPath = (org.eclipse.draw2dl.graph.Path) vertex.paths.get(i);
				if (!vPath.isMarked) {
	...
	private void recombineChildrenPaths() {
		...
		while (keyItr.hasNext()) {
			org.eclipse.draw2dl.graph.Path path = (org.eclipse.draw2dl.graph.Path) keyItr.next();

			path.fullReset();

			List childPaths = (List) pathsToChildPaths.get(path);
			org.eclipse.draw2dl.graph.Path childPath = null;

			for (int i = 0; i < childPaths.size(); i++) {
				childPath = (org.eclipse.draw2dl.graph.Path) childPaths.get(i);
	...
	private void recombineSubpaths() {
		for (int p = 0; p < orderedPaths.size(); p++) {
			org.eclipse.draw2dl.graph.Path path = (org.eclipse.draw2dl.graph.Path) orderedPaths.get(p);
	 ...
	 *         remaining paths
	 */
	public boolean removePath(org.eclipse.draw2dl.graph.Path path) {
	...
	private void resetObstacleExclusions() {
		for (int i = 0; i < userObstacles.size(); i++)
			((org.eclipse.draw2dl.graph.Obstacle) userObstacles.get(i)).exclude = false;
	}
	...
	private void resetVertices() {
		for (int i = 0; i < userObstacles.size(); i++) {
			org.eclipse.draw2dl.graph.Obstacle obs = (org.eclipse.draw2dl.graph.Obstacle) userObstacles.get(i);
			obs.reset();
		}
		for (int i = 0; i < workingPaths.size(); i++) {
			org.eclipse.draw2dl.graph.Path path = (org.eclipse.draw2dl.graph.Path) workingPaths.get(i);
	...
	private int solveDirtyPaths() {
		...
		for (int i = 0; i < userPaths.size(); i++) {
			org.eclipse.draw2dl.graph.Path path = (org.eclipse.draw2dl.graph.Path) userPaths.get(i);
			if (!path.isDirty)
				continue;
			List children = (List) pathsToChildPaths.get(path);
	 ...
	 * @param children
	 */
	private void refreshChildrenEndpoints(org.eclipse.draw2dl.graph.Path path, List children) {
		Point previous = path.getStartPoint();
		Point next;
		PointList bendpoints = path.getBendPoints();
		org.eclipse.draw2dl.graph.Path child;
		...
		for (int i = 0; i < children.size(); i++) {
			...
			child = (org.eclipse.draw2dl.graph.Path) children.get(i);
	 ...
	 * @param children
	 */
	private List regenerateChildPaths(org.eclipse.draw2dl.graph.Path path, List children,
		...
		while (currentSize < newSize) {
			org.eclipse.draw2dl.graph.Path child = new org.eclipse.draw2dl.graph.Path();
		...
		while (currentSize > newSize) {
			org.eclipse.draw2dl.graph.Path child = (org.eclipse.draw2dl.graph.Path) children.remove(children.size() - 1);
	 ...
	 */
	private int testOffsetSegmentForIntersections(org.eclipse.draw2dl.graph.Segment segment, int index,
                                                  org.eclipse.draw2dl.graph.Path path) {
		for (int i = 0; i < userObstacles.size(); i++) {
			org.eclipse.draw2dl.graph.Obstacle obs = (org.eclipse.draw2dl.graph.Obstacle) userObstacles.get(i);
			...
			if (vertex != null) {
				...
				org.eclipse.draw2dl.graph.Segment newSegmentStart = new org.eclipse.draw2dl.graph.Segment(segment.start, vertex);
				org.eclipse.draw2dl.graph.Segment newSegmentEnd = new org.eclipse.draw2dl.graph.Segment(vertex, segment.end);
	 ...
	 *            the obstacle
	 */
	private boolean testAndDirtyPaths(org.eclipse.draw2dl.graph.Obstacle obs) {
		boolean result = false;
		for (int i = 0; i < workingPaths.size(); i++) {
			org.eclipse.draw2dl.graph.Path path = (Path) workingPaths.get(i);
