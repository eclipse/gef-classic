 ...
 * @since 2.1.2
 */
class LocalOptimizer extends org.eclipse.draw2dl.graph.GraphVisitor {

	boolean shouldSwap(org.eclipse.draw2dl.graph.Node current, org.eclipse.draw2dl.graph.Node next) {
		if (org.eclipse.draw2dl.graph.GraphUtilities.isConstrained(current, next))
			return false;
		int crossCount = 0;
		int invertedCrossCount = 0;

		org.eclipse.draw2dl.graph.EdgeList currentEdges = current.incoming;
		EdgeList nextEdges = next.incoming;
		int rank = current.rank - 1;
		int iCurrent, iNext;

		for (int i = 0; i < currentEdges.size(); i++) {
			org.eclipse.draw2dl.graph.Edge currentEdge = currentEdges.getEdge(i);
	...
	private void swapNodes(org.eclipse.draw2dl.graph.Node current, org.eclipse.draw2dl.graph.Node next, org.eclipse.draw2dl.graph.Rank rank) {
		...
		do {
			...
			for (int r = 0; r < g.ranks.size(); r++) {
				...
				for (int n = 0; n < rank.count() - 1; n++) {
					org.eclipse.draw2dl.graph.Node currentNode = rank.getNode(n);
