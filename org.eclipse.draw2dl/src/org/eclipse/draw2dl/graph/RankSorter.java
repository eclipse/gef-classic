...
class RankSorter {
	...
	double progress;
	org.eclipse.draw2dl.graph.DirectedGraph g;
	...
	double evaluateNodeIncoming() {
		...
		org.eclipse.draw2dl.graph.EdgeList incoming = node.incoming;
		do {
			change = false;
			for (int i = 0; i < incoming.size() - 1; i++) {
				if (incoming.getSourceIndex(i) > incoming.getSourceIndex(i + 1)) {
					org.eclipse.draw2dl.graph.Edge e = incoming.getEdge(i);
	...
	public void sortRankIncoming(org.eclipse.draw2dl.graph.DirectedGraph g, org.eclipse.draw2dl.graph.Rank rank, int row,
                                 double progress) {
		this.currentRow = row;
		this.rank = rank;
	...
	public void init(org.eclipse.draw2dl.graph.DirectedGraph g) {
		this.g = g;
		for (int i = 0; i < g.ranks.size(); i++) {
			...
			Collections.sort(rank, new Comparator() {
				public int compare(Object left, Object right) {
					return ((org.eclipse.draw2dl.graph.Node) left).rowOrder - ((org.eclipse.draw2dl.graph.Node) right).rowOrder;
				}
			});
			postSort();
		}
	...
	boolean swap(int i) {
		org.eclipse.draw2dl.graph.Node left = rank.getNode(i);
		Node right = rank.getNode(i + 1);
		if (org.eclipse.draw2dl.graph.GraphUtilities.isConstrained(left, right))
			return false;
