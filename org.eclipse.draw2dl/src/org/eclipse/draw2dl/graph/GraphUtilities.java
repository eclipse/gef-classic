...
class GraphUtilities {
	...
	static org.eclipse.draw2dl.graph.Subgraph getCommonAncestor(org.eclipse.draw2dl.graph.Node left, org.eclipse.draw2dl.graph.Node right) {
		org.eclipse.draw2dl.graph.Subgraph parent;
		if (right instanceof org.eclipse.draw2dl.graph.Subgraph)
			parent = (org.eclipse.draw2dl.graph.Subgraph) right;
		else
	 ...
	 */
	public static boolean isCyclic(org.eclipse.draw2dl.graph.DirectedGraph graph) {
		return isCyclic(new org.eclipse.draw2dl.graph.NodeList(graph.nodes));
	}
	 ...
	 * @return whether the graph is cyclic
	 */
	public static boolean isCyclic(org.eclipse.draw2dl.graph.NodeList nodes) {
		if (nodes.isEmpty())
			return false;
		int size = nodes.size();
		...
		for (int i = 0; i < nodes.size(); i++) {
			...
			if (node.outgoing == null || node.outgoing.isEmpty()) { // this is a
				...
				for (int j = 0; j < node.incoming.size(); j++) {
					org.eclipse.draw2dl.graph.Edge e = node.incoming.getEdge(j);
	...
	public static int numberOfCrossingsInGraph(DirectedGraph graph) {
		...
		for (int i = 0; i < graph.ranks.size(); i++) {
			org.eclipse.draw2dl.graph.Rank rank = graph.ranks.getRank(i);
	...
	public static int numberOfCrossingsInRank(Rank rank) {
		...
		for (int i = 0; i < rank.size() - 1; i++) {
			org.eclipse.draw2dl.graph.Node currentNode = rank.getNode(i);
			org.eclipse.draw2dl.graph.Node nextNode;
			for (int j = i + 1; j < rank.size(); j++) {
				nextNode = rank.getNode(j);
				org.eclipse.draw2dl.graph.EdgeList currentOutgoing = currentNode.outgoing;
	...
	private static org.eclipse.draw2dl.graph.NodeList search(org.eclipse.draw2dl.graph.Node node, org.eclipse.draw2dl.graph.NodeList list) {
		if (node.flag)
			return list;
		node.flag = true;
		list.add(node);
	 ...
	 */
	public static boolean willCauseCycle(org.eclipse.draw2dl.graph.Node source, org.eclipse.draw2dl.graph.Node target) {
		org.eclipse.draw2dl.graph.NodeList nodes = search(target, new NodeList());
		nodes.resetFlags();
		return nodes.contains(source);
	}

	static boolean isConstrained(org.eclipse.draw2dl.graph.Node left, Node right) {
