...
class Vertex extends Point {
	...
	int count = 0;
	int totalCount = 0;
	org.eclipse.draw2dl.graph.Obstacle obs;
	List paths;
	 ...
	 */
	Vertex(int x, int y, org.eclipse.draw2dl.graph.Obstacle obs) {
		super(x, y);
		origX = x;
		origY = y;
		this.obs = obs;
	 ...
	 *            obstacle - can be null
	 */
	Vertex(Point p, org.eclipse.draw2dl.graph.Obstacle obs) {
		this(p.x, p.y, obs);
	}
	 ...
	 */
	void addPath(Path path, org.eclipse.draw2dl.graph.Segment start, org.eclipse.draw2dl.graph.Segment end) {
		if (paths == null) {
			paths = new ArrayList();
