...
public abstract class UpdateManager {
	...
	private org.eclipse.draw2dl.UpdateListener listeners[] = new org.eclipse.draw2dl.UpdateListener[0];
	 ...
	 */
	public abstract void addDirtyRegion(org.eclipse.draw2dl.IFigure figure, int x, int y, int w,
                                        int h);

	/**
	 * @see #addDirtyRegion(org.eclipse.draw2dl.IFigure, int, int, int, int)
	 */
	public void addDirtyRegion(org.eclipse.draw2dl.IFigure figure, Rectangle rect) {
	 ...
	 *            the invalid figure
	 */
	public abstract void addInvalidFigure(org.eclipse.draw2dl.IFigure figure);
	 ...
	 *            the listener to add
	 */
	public void addUpdateListener(org.eclipse.draw2dl.UpdateListener listener) {
		if (listener == null)
			throw new IllegalArgumentException();
		if (listeners == null) {
			listeners = new org.eclipse.draw2dl.UpdateListener[1];
			listeners[0] = listener;
		} else {
			int oldSize = listeners.length;
			org.eclipse.draw2dl.UpdateListener newListeners[] = new org.eclipse.draw2dl.UpdateListener[oldSize + 1];
	...
	protected void firePainting(Rectangle damage, Map dirtyRegions) {
		org.eclipse.draw2dl.UpdateListener localListeners[] = listeners;
	...
	/**
	 * Invoked by the {@link org.eclipse.draw2dl.LightweightSystem} (
	 ...
	 */
	public void removeUpdateListener(org.eclipse.draw2dl.UpdateListener listener) {
		if (listener == null)
			throw new IllegalArgumentException();
		for (int index = 0; index < listeners.length; index++)
			if (listeners[index] == listener) {
				int newSize = listeners.length - 1;
				org.eclipse.draw2dl.UpdateListener newListeners[] = null;
				if (newSize != 0) {
					newListeners = new org.eclipse.draw2dl.UpdateListener[newSize];
