 ...
 * 
 * @see org.eclipse.draw2dl.GridData
 * 
 */
public class GridLayout extends AbstractHintLayout {
	...
	org.eclipse.draw2dl.GridData getData(org.eclipse.draw2dl.IFigure[][] grid, int row, int column, int rowCount,
                                         int columnCount, boolean first) {
		org.eclipse.draw2dl.IFigure figure = grid[row][column];
		if (figure != null) {
			org.eclipse.draw2dl.GridData data = (org.eclipse.draw2dl.GridData) getConstraint(figure);
	 ...
	 * 
	 * @see
	 * org.eclipse.draw2dl.AbstractLayout#calculatePreferredSize(org.eclipse.
	 * draw2dl.IFigure, int, int)
	 */
	...
	Dimension layout(org.eclipse.draw2dl.IFigure container, boolean move, int x, int y, int width,
		...
		for (int j = 0; j < columnCount; j++) {
			for (int i = 0; i < rowCount; i++) {
				org.eclipse.draw2dl.GridData data = getData(grid, i, j, rowCount, columnCount, true);
				if (data != null) {
			...
			for (int i = 0; i < rowCount; i++) {
				org.eclipse.draw2dl.GridData data = getData(grid, i, j, rowCount, columnCount,
						false);
				if (data != null) {
					int hSpan = Math.max(1,
							Math.min(data.horizontalSpan, columnCount));
		...
		if (makeColumnsEqualWidth) {
			...
			if (width != SWT.DEFAULT && expandCount > 0) {
				...
				while (totalWidth != availableWidth) {
					...
					for (int j = 0; j < columnCount; j++) {
						for (int i = 0; i < rowCount; i++) {
							org.eclipse.draw2dl.GridData data = getData(grid, i, j, rowCount,
									columnCount, false);
							if (data != null) {
		...
		/* Wrapping */
		org.eclipse.draw2dl.GridData[] flush = null;
		int flushLength = 0;
				...
				for (int i = 0; i < rowCount; i++) {
					...
					if (data != null) {
						if (data.heightHint == SWT.DEFAULT) {
							org.eclipse.draw2dl.IFigure child = grid[i][j];
							// TEMPORARY CODE
							int hSpan = Math.max(1,
									Math.min(data.horizontalSpan, columnCount));
							int currentWidth = 0;
							...
							if ((currentWidth != data.cacheWidth && data.horizontalAlignment == SWT.FILL)
								...
								if (flush == null)
									flush = new org.eclipse.draw2dl.GridData[children.size()];
			...
			for (int i = 0; i < rowCount; i++) {
				...
				for (int j = 0; j < columnCount; j++) {
					...
					if (data != null) {
						...
						case SWT.CENTER:
						case org.eclipse.draw2dl.GridData.CENTER:
							childX = gridX
									+ Math.max(0, (cellWidth - childWidth) / 2);
							break;
						case SWT.RIGHT:
						case SWT.END:
						case org.eclipse.draw2dl.GridData.END:
							childX = gridX
	 ...
	 * 
	 * @see
	 * org.eclipse.draw2dl.LayoutManager#getConstraint(org.eclipse.draw2dl.IFigure
	 * )
	 */
