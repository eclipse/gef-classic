 ...
 * the client using {@link org.eclipse.draw2dl.geometry.Point Points}. SWT uses integer arrays when painting
 * polylines and polygons.
 */
public class PointList implements java.io.Serializable, Translatable {

	private int[] points = new int[0];
	private org.eclipse.draw2dl.geometry.Rectangle bounds;
	private int size = 0;
	 ...
	 * @since 2.0
	 */
	public void addPoint(org.eclipse.draw2dl.geometry.Point p) {
		addPoint(p.x, p.y);
	}
	 ...
	 * @since 2.0
	 */
	public org.eclipse.draw2dl.geometry.Rectangle getBounds() {
		if (bounds != null)
			return bounds;
		bounds = new org.eclipse.draw2dl.geometry.Rectangle();
		if (size > 0) {
	 ...
	 * @since 2.0
	 */
	public org.eclipse.draw2dl.geometry.Point getFirstPoint() {
		return getPoint(0);
	}
	 ...
	 * @since 2.0
	 */
	public org.eclipse.draw2dl.geometry.Point getLastPoint() {
	 ...
	 *             if the list is empty
	 */
	public org.eclipse.draw2dl.geometry.Point getMidpoint() {
		if (size() % 2 == 0)
	 ...
	 * @since 2.0
	 */
	public org.eclipse.draw2dl.geometry.Point getPoint(int index) {
		if (index < 0 || index >= size)
			throw new IndexOutOfBoundsException("Index: " + index + //$NON-NLS-1$
					", Size: " + size); //$NON-NLS-1$
		index *= 2;
		return new org.eclipse.draw2dl.geometry.Point(points[index], points[index + 1]);
	}
	 ...
	 * @since 2.0
	 */
	public org.eclipse.draw2dl.geometry.Point getPoint(org.eclipse.draw2dl.geometry.Point p, int index) {
		if (index < 0 || index >= size)
			throw new IndexOutOfBoundsException("Index: " + index + //$NON-NLS-1$
					", Size: " + size); //$NON-NLS-1$
		index *= 2;
	 ...
	 * @see #setPoint(org.eclipse.draw2dl.geometry.Point, int)
	 * @since 2.0
	 */
	public void insertPoint(org.eclipse.draw2dl.geometry.Point p, int index) {
	...
	public boolean intersects(Rectangle r) {
		...
		for (int i = 0; i < (size - 1) * 2; i += 2) {
			if (org.eclipse.draw2dl.geometry.Geometry.linesIntersect(diagonal1x1, diagonal1y1, diagonal1x2,
					diagonal1y2, points[i], points[i + 1], points[i + 2],
					points[i + 3])
					|| org.eclipse.draw2dl.geometry.Geometry.linesIntersect(diagonal2x1, diagonal2y1,
	...
	/**
	 * @see org.eclipse.draw2dl.geometry.Translatable#performTranslate(int, int)
	 */
	 ...
	 * 
	 * @since 2.0
	 * @see #addPoint(org.eclipse.draw2dl.geometry.Point)
	 * @param index
	 ...
	 */
	public org.eclipse.draw2dl.geometry.Point removePoint(int index) {
		bounds = null;
		if (index < 0 || index >= size)
			throw new IndexOutOfBoundsException("Index: " + index + //$NON-NLS-1$
					", Size: " + size); //$NON-NLS-1$
		...
		index *= 2;
		org.eclipse.draw2dl.geometry.Point pt = new org.eclipse.draw2dl.geometry.Point(points[index], points[index + 1]);
		if (index != size * 2 - 2)
	 ...
	 * @since 2.0
	 */
	public void setPoint(org.eclipse.draw2dl.geometry.Point pt, int index) {
		if (index < 0 || index >= size)
			throw new IndexOutOfBoundsException("Index: " + index + //$NON-NLS-1$
					", Size: " + size); //$NON-NLS-1$
	 ...
	 * @since 2.0
	 */
	public final void translate(org.eclipse.draw2dl.geometry.Point pt) {
		translate(pt.x, pt.y);
	}
	 ...
	 *         PointList
	 * @see org.eclipse.draw2dl.geometry.Geometry#polygonContainsPoint(PointList, int, int)
	 * @since 3.5
	 */
	public boolean polygonContainsPoint(int x, int y) {
		return org.eclipse.draw2dl.geometry.Geometry.polygonContainsPoint(this, x, y);
	}
	 ...
	 * @see org.eclipse.draw2dl.geometry.Geometry#polylineContainsPoint(PointList, int, int, int)
	 * @since 3.5
	 */
