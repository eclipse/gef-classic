...
/**
 * Provides a {@link org.eclipse.draw2dl.Connection} with an orthogonal route between the
...
public final class ManhattanConnectionRouter extends AbstractRouter {
	...
	private int getColumnNear(org.eclipse.draw2dl.Connection connection, int r, int n, int x) {
	...
	protected Ray getEndDirection(org.eclipse.draw2dl.Connection conn) {
		org.eclipse.draw2dl.ConnectionAnchor anchor = conn.getTargetAnchor();
	...
	protected int getRowNear(org.eclipse.draw2dl.Connection connection, int r, int n, int x) {
		int min = Math.min(n, x), max = Math.max(n, x);
		if (min > r) {
			max = min;
			min = r - (min - r);
	...
	protected Ray getStartDirection(org.eclipse.draw2dl.Connection conn) {
	...
	protected void processPositions(Ray start, Ray end, List positions,
			boolean horizontal, org.eclipse.draw2dl.Connection conn) {
	...
	protected void removeReservedLines(org.eclipse.draw2dl.Connection connection) {
	...
	protected void reserveColumn(org.eclipse.draw2dl.Connection connection, Integer column) {
	...
	protected void reserveRow(org.eclipse.draw2dl.Connection connection, Integer row) {
		ReservedInfo info = (ReservedInfo) reservedInfo.get(connection);
		if (info == null) {
			info = new ReservedInfo();
			reservedInfo.put(connection, info);
	...
	/**
	 * @see ConnectionRouter#route(org.eclipse.draw2dl.Connection)
	 */
