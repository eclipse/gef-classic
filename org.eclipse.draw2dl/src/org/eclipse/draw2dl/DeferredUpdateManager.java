 ...
 */
public class DeferredUpdateManager extends org.eclipse.draw2dl.UpdateManager {
	...
	private org.eclipse.draw2dl.GraphicsSource graphicsSource;
	 ...
	 *            the graphics source
	 */
	public DeferredUpdateManager(org.eclipse.draw2dl.GraphicsSource gs) {
	 ...
	 */
	public synchronized void addDirtyRegion(org.eclipse.draw2dl.IFigure figure, int x, int y,
                                            int w, int h) {
	 ...
	 *            the invalid figure
	 */
	public synchronized void addInvalidFigure(org.eclipse.draw2dl.IFigure f) {
	 ...
	 */
	protected org.eclipse.draw2dl.Graphics getGraphics(Rectangle region) {
	...
	protected void paint(GC gc) {
		if (!validating) {
			org.eclipse.draw2dl.SWTGraphics graphics = new SWTGraphics(gc);
			if (!updating) {
				/**
	...
	/**
	 * @see org.eclipse.draw2dl.UpdateManager#performValidation()
	 */
	public synchronized void performValidation() {
		...
		try {
			...
			for (int i = 0; i < invalidFigures.size(); i++) {
				fig = (org.eclipse.draw2dl.IFigure) invalidFigures.get(i);
	 ...
	 *            the graphics object
	 */
	protected void releaseGraphics(org.eclipse.draw2dl.Graphics graphics) {
		graphics.dispose();
	 ...
	 * {@link org.eclipse.draw2dl.UpdateManager#firePainting(Rectangle, Map)}, unless there are no
	 * dirty regions.
	 */
	protected void repairDamage() {
		Iterator keys = dirtyRegions.keySet().iterator();
		Rectangle contribution;
		org.eclipse.draw2dl.IFigure figure;
		org.eclipse.draw2dl.IFigure walker;

		while (keys.hasNext()) {
			figure = (org.eclipse.draw2dl.IFigure) keys.next();
