...
final class DotDrawer {
    ...
    public static File renderImage(final File dotExecutableDir, final File dotInputFile, final String format) {
        ...
        String inputFolder = new File(dotInputFile.getParent()).getAbsolutePath() + File.separator;
        String outputFolder = inputFolder;
        String dotExecutable = "dot" + (runningOnWindows() ? ".exe" : "");
        String[] commands =
                new String[] { dotExecutableDir.getAbsolutePath() + File.separator + dotExecutable,
                        outputFormat, "-o", outputFolder + resultFile, inputFolder + dotFile };
        call(commands);
