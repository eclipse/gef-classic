...
public class IndividualCharacters {
	...
	public static void main(String[] args) {
		...
		final String message = "\u202e\u0634\u0636\u202c\u0634"; //$NON-NLS-1$
		...
		canvas.setText("Left to Right"); //$NON-NLS-1$
		...
		canvas2.setText("Right to Left"); //$NON-NLS-1$
		...
		PaintListener pl = e -> {
			...
			for (int i = 0; i < rtol.length(); i++) {
				...
				e.gc.drawString("" + layout.getLevel(i), where.x, where.y + 48 + ((i % 2) == 0 ? 0 : 12), true); //$NON-NLS-1$
			}
			layout.dispose();
		};
