...
public class ResizeNodeConstraintCommand extends Command {
	...
	private final GraphNodeEditPart editPart;
	
	private boolean allowOverlap;
	private boolean enforceBounds;
	
	private final NoOverlapLayout noOverlapLayout;

	
	/**
	 * Create a command that can resize and/or move a node.
	 * @param node the node to mode/resize
	 * @param req  the move and resize request
	 * @param newBounds the new size and location
	 * @throws IllegalArgumentException if any of the parameters is null
	 */
	public ResizeNodeConstraintCommand(GraphNodeEditPart editPart, GraphModelNode node, ChangeBoundsRequest req, Rectangle newBounds) {
	...
	public void redo() {
		...
		if (enforceBounds) {
			Rectangle rect = editPart.getFigure().getParent().getBounds();
		...
		if (!allowOverlap) {
			noOverlapLayout.checkBounds(editPart, newBounds);
		} else {
			// move the current editpart's figure to last in the list to put it on top
			IFigure fig = editPart.getFigure();
			fig.getParent().getChildren().remove(fig);
			fig.getParent().getChildren().add(fig);
		}
		
		node.setLocationInLayout(newBounds.getLocation().x, newBounds.getLocation().y);
	}
	...
	public void undo() {
		...
		node.setLocationInLayout(oldBounds.getLocation().x, oldBounds.getLocation().y);
	}

	/**
	 * Sets if overlapping nodes are allowed.  
	 * @param allowOverlap
	 */
	...
	/**
	 * Sets if the bounds are enforced.  If a node is moved outside the bounds
	 * it will be placed at the boundary.
	 * @param enforceBounds
	 */
